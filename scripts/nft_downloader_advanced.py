...
class AdvancedNFTDownloader:
    ...
    def __init__(self, output_dir="nfts", workers=1, gateway_url=None):
        ...
        # Identifiers handled by THIS run (separate from `downloaded` which
        # also gets seeded with every existing file in the output dir for
        # skip-on-rerun). Only these get tagged into the source map so we
        # don't steal entries from other collections.
        self.run_identifiers = set()
    ...
    def _persist_source_map(self):
        """Persist the filename → source-CSV mapping immediately.
        Previously this only ran at the very end of a complete download,
        so interrupted downloads (kiosk reboot, manual stop, partial
        failures) left zero source-map entries — which broke the
        Library "Remove files" action because Method 1 (source-map
        lookup) found nothing for that collection. Now called from
        _save_progress() so the map reflects every file actually on disk
        even when the download is mid-flight."""
        if not self.source_csv:
            return
        if not (self.run_identifiers or self.saved_files):
            return
        ...
        try:
            ...
            source_map = {}
        extensions = [".gif", ".png", ".jpg", ".jpeg", ".mp4", ".glb",
                      ".html", ".bin", ".webp", ".svg", ".avif", ".json", ".txt"]
        for identifier in list(self.run_identifiers):
            ...
            if not found:
                for ext in extensions:
                    for match in self.output_dir.glob(f"{identifier}_*{ext}"):
                        source_map[match.name] = self.source_csv
        for fname in list(self.saved_files):
    ...
    def _save_progress(self):
        ...
        # Also persist the source map incrementally so interrupted
        # downloads still leave a usable filename → CSV mapping behind.
        # Done outside the lock — _persist_source_map only reads
        # self.run_identifiers / self.saved_files / self.failed, which
        # are append-only structures.
        self._persist_source_map()
    ...
    def download_nft(self, contract, token_id, chain='ethereum', image_url_fallback=''):
        ...
        with self.lock:
            self.run_identifiers.add(identifier)
    ...
    def process_csv(self, csv_path):
        ...
        # Final source-map persist. Same logic now lives in
        # _persist_source_map() (also invoked by _save_progress so
        # interrupted downloads still leave a usable mapping). Call once
        # more here as a belt-and-suspenders flush at the very end.
        self._persist_source_map()
