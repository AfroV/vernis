...
try:
    ...
    if 'lab.html' in url:
        ...
        ws.send(json.dumps({'id':2,'method':'Page.navigate','params':{'url':'http://127.0.0.1/index.html'}}))
    ws.settimeout(10)
    r = json.loads(ws.recv())
    ws.close()
...
for p in pages:
    url = p.get('url', '')
    if ('localhost' in url or '127.0.0.1' in url) and p.get('webSocketDebuggerUrl'):
