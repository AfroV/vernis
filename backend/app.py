                    ...
                    with open(path, 'r') as f:
                        ...
                        for row in reader:
                            ...
                            if cid and cid not in ["See CSV", "On-Chain", "Arweave", "--"]:
                                cid = cid.replace('ipfs://', '').strip()
                                # CIDs sometimes have a path suffix (e.g.
                                # "bafy.../thumbnail" for directory CIDs).
                                # The downloader stores files named after the
                                # bare CID, so strip the path before globbing.
                                cid = cid.split('/', 1)[0]
                                if cid.startswith('Qm') or cid.startswith('bafy'):
                                    cids_to_delete.append(cid)
                    break
...
def upload_csv_to_library():
    ...
    try:
        ...
        # Optional wallet origin — when supplied, the collection shows the
        # Auto-sync toggle in Library and appears in Settings → Wallets.
        wallet = request.form.get('wallet', '').strip()
        chains_raw = request.form.get('chains', '').strip()
        chains = [c.strip() for c in chains_raw.split(',') if c.strip()] if chains_raw else []

        # Save metadata if provided, OR if wallet origin info exists (so the
        # autoupdate toggle is discoverable in Library/Settings)
        if name or description or wallet:
            meta_path = csv_path.with_suffix('.json')
            metadata = {
...
def _write_privileged(path, content):
    ...
    /opt/vernis can be replaced by unlinking (parent dir is afrol-writable). Sysfs trip points
