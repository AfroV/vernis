...
# (max_failures_for_this_tier, cooldown_seconds)
# 1-6 free, 7-10 → 30s, 11-15 → 2 min, 16+ → 1 hour
PER_IP_COOLDOWN_SCHEDULE = [(6, 0), (10, 30), (15, 120), (None, 3600)]
FAILURE_WINDOW_HOURS = 24  # rolling window for counting failures per IP
...
def _unique_tmp(path):
    """Per-writer tmp path so concurrent saves never collide on os.replace().
    Without this, two writers both targeting <path>.tmp will race: the first
    os.replace consumes the tmp, the second sees FileNotFoundError → 500."""
    import secrets as _s
    return path.with_suffix(path.suffix + f".tmp.{os.getpid()}.{_s.token_hex(4)}")
...
def save_security_config(cfg):
    ...
    tmp = _unique_tmp(SECURITY_CONFIG_FILE)
...
def hash_pin(pin):
    ...
    return _bcrypt.hashpw(
        pin.encode("utf-8"),
        _bcrypt.gensalt(rounds=PIN_BCRYPT_COST),
    ).decode("utf-8")
...
def issue_session(ip, ua, permanent=False):
    """Issue a session token.
    permanent=True → no expiry; only ends via explicit sign-out or
    PIN/mode change. Surfaced as 'Trust this browser until I sign out'
    in the UI."""
    token = _secrets.token_urlsafe(32)
    now = time.time()
    expires_at = None if permanent else now + (SESSION_TTL_DAYS * 86400)
...
def list_sessions():
    ...
    for token, entry in sessions.items():
        ...
        if permanent or (entry.get("expires_at") and entry["expires_at"] >= now):
            cleaned[token] = entry
            out.append({
                "token_id": token[:8],
                "ip": entry.get("ip", ""),
...
def _load_failures():
    try:
        ...
        # Drop legacy fields silently if present in older files.
        data.pop("global", None)
        data.pop("hard_locked_at", None)
        return data
    except (json.JSONDecodeError, OSError):
        return {"by_ip": {}}
...
def _cooldown_for_count(count):
    """Cooldown seconds that an IP at this failure count must wait before
    its NEXT attempt is accepted. count=0 means no failures yet."""
...
def attempts_count(ip):
    """How many failures this IP has logged in the rolling window."""
    data = _load_failures()
    window = FAILURE_WINDOW_HOURS * 3600
    return len(_purge_old(data["by_ip"].get(ip, []), window))


def next_cooldown(ip):
    """Cooldown that the NEXT failure from this IP would trigger.
    Drives the 'next wrong PIN will trigger…' warning copy."""
    return _cooldown_for_count(attempts_count(ip) + 1)


def record_failure(ip):
    """Log one failed attempt. Returns the cooldown that now applies
    to this IP before its next attempt is accepted."""
    now = time.time()
    data = _load_failures()
    window = FAILURE_WINDOW_HOURS * 3600
    by_ip = data["by_ip"]
...
_DELETE_EXACT = frozenset([
    ...
    # Destructive system ops — must require PIN session in Mode B/C.
    # Without this, the older token-based require_auth() blocked all
    # remote (phone/PC) callers because /api/auth-token is localhost-only.
    "/api/reboot",
    "/api/shutdown",
    "/api/update",     # legacy /opt/vernis/scripts/updater.sh trigger
    "/api/https",      # HTTPS opt-in / cert install / Caddyfile rewrite
    "/api/wifi",       # WiFi network change — can lock user out remotely
])
...
def _enforce_security():
    ...
    1. Kiosk (localhost) — always allow.
    2. Classify path; READ + BOOTSTRAP always allow.
    3. SECURITY endpoints handle their own auth.
    4. Mode A — allow everything.
    ...
    if cls in ("read", "bootstrap"):
        return None
    if cls == "security":
        return None  # endpoint implements its own auth
...
# ========================================
# Security: /api/security/* endpoints
# ========================================
...
def security_config():
    ...
    is_kiosk = ip in ("127.0.0.1", "::1")
    remaining = cooldown_remaining(ip)
    locked_until = (time.time() + remaining) if remaining > 0 else None
    # days since security.json was created, for the 7-day nudge
    try:
        created = cfg.get("created_at", "")
        ts = datetime.strptime(created, "%Y-%m-%dT%H:%M:%SZ") if created else datetime.utcnow()
        days_since_setup = max(0, (datetime.utcnow() - ts).days)
    except (ValueError, TypeError):
        days_since_setup = 0
    return jsonify({
        "mode": cfg.get("mode", "A"),
        ...
        "attempts_count": attempts_count(ip),
        "next_cooldown": next_cooldown(ip),
...
def security_login():
    ...
    if not pin_hash or not verify_pin(pin, pin_hash):
        ...
        # After recording, expose updated counters + the cooldown that
        # the NEXT failure would trigger (drives the "next wrong PIN
        # will trigger…" warning in the UI).
        return jsonify({
            "error": "invalid_pin",
            "attempts_count": attempts_count(ip),
            "next_cooldown": next_cooldown(ip),
            "cooldown_remaining": cooldown_remaining(ip),
        }), 401

    clear_failures(ip)
    permanent = bool(data.get("trust_until_signout"))
    sess = issue_session(
        ip,
...
@app.route("/api/security/verify-owner", methods=["POST"])
def security_verify_owner():
    """Verify the owner (device) password without side effects on PIN/mode.
    Used by the recovery modal to surface a wrong-password error on Step 1
    instead of forcing the user through a 6-digit PIN entry first.

    Counts toward per-IP cooldown so this can't be used to brute-force
    the owner password any faster than the recover endpoint itself.
    """
    ip = request.remote_addr or ""
    remaining = cooldown_remaining(ip)
    if remaining > 0:
    ...
    if not owner_hash or not verify_owner_password(pwd, owner_hash):
        record_failure(ip)
        append_audit("verify_owner", ip=ip, result="fail")
        return jsonify({
            "error": "invalid_owner_password",
            "attempts_count": attempts_count(ip),
            "next_cooldown": next_cooldown(ip),
    ...
    clear_failures(ip)
    append_audit("verify_owner", ip=ip, result="ok")
    return jsonify({"ok": True})
...
@app.route("/api/security/sessions/others", methods=["DELETE"])
def security_sessions_revoke_others():
    """Sign out everyone else — keep the caller's session, drop the rest."""
    err = _require_session_or_401()
    if err:
        return err
    current = request.headers.get("X-Vernis-PIN-Session", "")
    sessions = _load_sessions()
    kept = {current: sessions[current]} if current in sessions else {}
    removed = len(sessions) - len(kept)
    _save_sessions(kept)
    append_audit("sessions_revoke_others", ip=request.remote_addr or "",
...
_PIN_CID_RE = re.compile(r'pinned\s+(Qm[a-zA-Z0-9]{44}|baf[a-z2-7]{50,})\s+recursively', re.I)


def _extract_pinned_cid(stdout, fallback_cid=""):
    """Parse `ipfs pin add` stdout to extract the actual CID that was pinned.

    For a path-style CID (Qm.../file.png) IPFS resolves the path and pins
    the LEAF content's hash, not the wrapper. The leaf hash appears in
    stdout as "pinned <CID> recursively". Returns the leaf CID if a single
    one is found, or fallback_cid if parsing fails.
    """
    if not stdout:
        return fallback_cid
    m = _PIN_CID_RE.search(stdout)
    return m.group(1) if m else fallback_cid
...
def collection_failure_report(filename):
    ...
    # Pin worker outcomes — leaf-pinned and unreachable-wrapper info live
    # in the CSV sidecar + pin progress file, not in download_progress.json.
    # Compute these up front so they're available even when download_progress
    # is for a different CSV (the common case once the downloader has moved
    # on to another collection).
    leaf_pins_persistent = {}
    try:
        sidecar_path = (CSV_LIBRARY_DIR / filename).with_suffix(".json")
        if sidecar_path.exists():
            sm = json.loads(sidecar_path.read_text())
            leaf_pins_persistent = sm.get("leaf_pins", {}) or {}
    except Exception:
        pass

    pin_unreachable = []
    pin_progress_file = TMP_DIR / f"pin_progress_{re.sub(r'[^\w\-\.]', '_', filename).replace('.', '_')}.json"
    try:
        if pin_progress_file.exists():
            pp = json.loads(pin_progress_file.read_text())
            pin_unreachable = pp.get("unreachable_wrappers", []) or []
    except Exception:
        pass

    leaf_list = [{"wrapper_cid": k, **v} for k, v in leaf_pins_persistent.items()]
    pin_block_full = {
        "leaf_pinned": {
            "count": len(leaf_list),
            "items": leaf_list[:20] if include_details else [],
        },
        "unreachable_wrappers": {
            "count": len(pin_unreachable),
            "items": pin_unreachable[:20] if include_details else [],
        },
    }
    ...
    # Carry pin-block (computed earlier) into the live result.
    result.update(pin_block_full)
...
def https_setup():
        ...
        try:
            ...
            if os.path.exists(CADDYFILE):
                with open(CADDYFILE, 'r') as f:
                    content = f.read()
                    https_active = ("tls " + CERT_PATH) in content
    ...
    if request.method == "DELETE":
        ...
        # Auth handled by PIN middleware (path is in _DELETE_EXACT).
        try:
    ...
    # POST - enable HTTPS (auth via PIN middleware)
    try:
        import subprocess as _sp
        import re as _re
        ...
        # Detect whether the active Caddyfile uses the new LAN-only structure
        # (presence of `import lan_only`). When it does, the appended :80
        # redirect must use the LAN-gated snippet, and ufw must open :443
        # only from the LAN CIDR — otherwise enabling HTTPS would silently
        # expose :443 to the public internet.
        uses_lan_only = "import lan_only" in caddy_content
        redirect_block = (
            "\n\n:80 {\n\timport lan_only_redir\n}\n"
            if uses_lan_only else
        ...
        caddy_content = _re.sub(r'\n*:80\s*\{\s*import\s+lan_only_redir\s*\}\s*$', '', caddy_content).rstrip()

        # Build new content: :443 block with tls + original directives,
        # then the (possibly LAN-gated) :80 redirect.
        new_content = pattern.sub(
            ":443 {\n\ttls " + CERT_PATH + " " + KEY_PATH,
            caddy_content,
            count=1
        )
        new_content += redirect_block
        ...
        # If the device has been hardened (network-state.json present), restrict
        # :443 to the LAN CIDR — never expose the admin UI publicly when
        # the owner enables HTTPS.
        ...
        if ufw_check.returncode == 0 and "active" in ufw_check.stdout.lower():
            lan_cidr = None
            try:
                with open("/opt/vernis/network-state.json", "r") as nf:
                    lan_cidr = json.load(nf).get("lan_cidr")
            except Exception:
                pass
            if not lan_cidr:
                ...
                try:
                    out = _sp.run(["sh", "-c",
                        "ip -o -4 addr show $(ip route show default | awk '/default/ {print $5; exit}') "
                        "| awk '{print $4; exit}'"],
                        capture_output=True, text=True, timeout=5).stdout.strip()
                    if out:
                        # round to /24
                        ip = out.split("/")[0].split(".")
                        if len(ip) == 4:
                            lan_cidr = f"{ip[0]}.{ip[1]}.{ip[2]}.0/24"
                except Exception:
                    pass

            if uses_lan_only and lan_cidr:
                # Drop any prior 'allow 443/tcp from Anywhere' rule, then add LAN-only
                _sp.run(["ufw", "delete", "allow", "443/tcp"], capture_output=True, text=True, timeout=10)
                _sp.run(["ufw", "allow", "from", lan_cidr, "to", "any",
                         "port", "443", "proto", "tcp",
                         "comment", "Vernis HTTPS (LAN only)"],
                        capture_output=True, text=True, timeout=10)
            else:
                # Legacy install (no hardening yet) — match previous behavior
...
def change_wifi():
    ...
    Auth is handled by the PIN middleware (_enforce_security): this path
    is in _DELETE_EXACT, so Mode A allows it openly while Mode B and
    Mode C require a PIN session.

    nmcli connection.add/up require polkit admin; we use the NOPASSWD
    sudo entry the installer creates (`<user> ALL=(ALL) NOPASSWD:
    /usr/bin/nmcli *`). Without sudo here, even valid auth still got
    'insufficient privileges' back from NetworkManager.
    """
    try:
        data = request.json
        ...
        uuid_result = subprocess.run(
            ["sudo", "-n", "nmcli", "-t", "-f", "NAME,UUID", "connection", "show"],
        ...
        result = subprocess.run(
            ["sudo", "-n", "nmcli", "connection", "up", up_target],
        ...
        if result.returncode == 0:
            ...
            subprocess.run(["sudo", "-n", "nmcli", "connection", "delete", up_target],
                          capture_output=True, env=nmcli_env, timeout=5)
...
def update():
    ...
    Auth via PIN middleware (path is in _DELETE_EXACT).
    """
    try:
...
def reboot():
    ...
    Auth is handled by the PIN middleware (`_enforce_security`): this
    path is in `_DELETE_EXACT`, so Mode A allows it openly while Mode B
    and Mode C require a PIN session.
    """
    try:
    ...
    try:
        ...
        for csv_file in CSV_LIBRARY_DIR.glob("*.csv"):
            ...
            collections.append({
                ...
                # Default true preserves prior behavior — auto-synced art goes
                # straight to the gallery unless the user opts out per wallet.
                "auto_add_to_gallery": metadata.get("auto_add_to_gallery", True),
...
def csv_collection_status(filename):
    ...
    try:
        ...
        active_nft_dir = get_active_nft_dir()
        downloaded = 0
        # Media we surface in the gallery
        MEDIA_EXTS = {'.jpg', '.jpeg', '.png', '.gif', '.svg', '.webp', '.mp4', '.html', '.avif'}
        # Any extension a downloaded IPFS CID might land under — JSON metadata
        # blobs and raw .bin payloads are legitimate "downloaded" content even
        # though we don't render them in the gallery.
        DOWNLOADED_EXTS = MEDIA_EXTS | {'.json', '.bin', '.glb', '.txt'}
        ...
        # CID matching — count unique CIDs whose content has been saved to disk,
        # regardless of file extension. A CID resolved into a JSON metadata file
        # or a raw blob still represents a successful download.
        downloaded_cids = 0
        if cids:
            local_stems = set()
            for f in active_nft_dir.iterdir():
                if f.suffix.lower() in DOWNLOADED_EXTS:
                    stem = f.stem.split('_')[0] if '_' in f.stem else f.stem
                    local_stems.add(stem)
                    local_stems.add(f.stem)
        ...
        # Count IPFS-pinnable items for this collection. Match the pin
        # worker's view so percent_pinned can actually reach 100%:
        #   - every CID-stemmed file mapped to this CSV (any extension —
        #     metadata JSONs are pinnable too)
        #   - every CID listed in the CSV
        ...
        # Pull leaf-pin map from sidecar — wrapper CIDs we couldn't pin
        # directly but whose local leaf content we did pin (the wrapper is
        # unreachable on IPFS right now, but the user's content is preserved).
        # Format: { wrapper_cid: { leaf_cid, filename, pinned_at, ... } }
        leaf_pins_map = {}
        try:
            sidecar_path = (CSV_LIBRARY_DIR / filename).with_suffix('.json')
            if sidecar_path.exists():
                sm = json.loads(sidecar_path.read_text())
                leaf_pins_map = sm.get("leaf_pins", {}) or {}
        except Exception:
            pass

        # Check which are pinned. A CID counts as pinned when either:
        #   - the wrapper CID is recursively pinned on this node, OR
        #   - we previously pinned its local content under a leaf CID and
        #     that leaf CID is still in the pin set (preserved-as-leaf).
        pinned = 0
        leaf_pinned = 0
            ...
            if result.returncode == 0:
                ...
                for base_cid in set(ipfs_media_in_collection):
                    if base_cid in pinned_cids:
                        pinned += 1
                    elif base_cid in leaf_pins_map and \
                            leaf_pins_map[base_cid].get("leaf_cid") in pinned_cids:
                        leaf_pinned += 1
        except:
            pass

        total = effective_total
        # The user's content is preserved either way, so both pin types count
        # toward "effectively pinned" for percentage purposes.
        effective_pinned = (pinned + leaf_pinned) if ipfs_available else downloaded

        # Check for last download failure info from progress file
        failed_count = 0
        failed_summary = ""
        failed_buckets = {"likely": 0, "unlikely": 0}
        if downloaded < total:
            try:
                active_nft_dir = get_active_nft_dir()
                progress_file = active_nft_dir / "download_progress.json"
                ...
                if progress_file.exists():
                    ...
                    if src and src in filename:
                        ...
                        if isinstance(failed_data, dict):
                            ...
                            # Classify each error into likely/unlikely buckets
                            for e in errors:
                                bucket, _reason = _classify_download_error(str(e))
                                if bucket == "likely":
                                    failed_buckets["likely"] += 1
                                else:
                                    failed_buckets["unlikely"] += 1
                            # Pick a human summary label — most common reason
                            ...
                            failed_count = len(failed_data)
                            failed_buckets["unlikely"] = failed_count
            except:
                pass
        ...
        # Count of CIDs the pin worker couldn't pin even via leaf — read
        # from the pin progress file so the card can surface a "Why?" link
        # when there's something useful to explain, even on collections
        # where every download succeeded.
        unreachable_count = 0
        try:
            pp_file = TMP_DIR / f"pin_progress_{re.sub(r'[^\w\-\.]', '_', filename).replace('.', '_')}.json"
            if pp_file.exists():
                pp = json.loads(pp_file.read_text())
                unreachable_count = len(pp.get("unreachable_wrappers", []) or [])
        except Exception:
            pass
        ...
        return jsonify({
            ...
            "pinned_wrapper": pinned,
            "pinned_leaf": leaf_pinned,
            # Raw count of recorded wrapper→leaf pins, regardless of whether
            # the wrapper happens to be in this collection's expected set.
            # Used by the UI to decide whether to surface a "Why?" chevron.
            "leaf_pin_records": len(leaf_pins_map),
    ...
    try:
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                cid = cid.replace('ipfs://', '').strip()
                if cid and (cid.startswith('Qm') or cid.startswith('bafy')) and cid not in seen:
                    cids.append(cid)
                    seen.add(cid)
        ...
        # Extend with disk-mapped CIDs from the source map. The downloader may
        # have pulled in metadata-referenced media (e.g. <CID>_<tokenid>.json
        # files that point at additional image CIDs) that aren't in the CSV
        # but ARE part of this collection. The status endpoint counts these
        # as pinnable, so the pin worker has to try them too — otherwise
        # percent_pinned can never reach 100%.
        try:
            for sdir in search_dirs:
                src_map_file = sdir / "nft-source-map.json"
                if not src_map_file.exists():
                    continue
                src_map = json.loads(src_map_file.read_text())
                for fname, src in src_map.items():
                    if src != filename:
                        continue
                    stem = Path(fname).stem
                    base = stem.split('_')[0] if '_' in stem else stem
                    if (base.startswith('Qm') or base.startswith('bafy')) and base not in seen:
                        cids.append(base)
                        seen.add(base)
        except Exception:
            pass
        ...
        error_count = 0
        error_list = []
        # First-pass timeouts/errors get retried with longer timeouts before
        # being declared a final error. Each entry is
        # (cid, base_cid, prev_err_msg, local_file_path_or_None).
        retry_queue = []
        # When a wrapper CID is genuinely unreachable on the IPFS network
        # but we have a local leaf file, we pin the leaf's content CID as a
        # fallback. Format: { wrapper_cid: {"leaf_cid":..., "filename":...,
        # "pinned_at": iso} }. Persisted to the CSV sidecar JSON so the
        # status endpoint can count these as effectively pinned across runs.
        leaf_pins_run = {}
        unreachable_wrappers = []  # CIDs we couldn't pin even via leaf
        ...
        for i, cid in enumerate(cids):
            ...
            # Find local file. The downloader saves files under several patterns:
            #   <CID>.<ext>                — bare CID
            #   <CID>_<suffix>.<ext>       — when an HTTP URL had a path component
            #                                 (e.g. ipfs://<CID>/asset.gif → <CID>_asset.gif)
            #                                 or when multiple tokens share a directory CID
            #                                 (e.g. <CID>_<tokenid>.json)
            # We accept any of these — they all derive from the same content-addressed
            # source and pinning any one of them serves the original CID.
            local_file = None
            ...
            for search_dir in search_dirs:
                ...
                if local_file:
                    break
                # Suffix variants: <CID>_*.<ext>. Pick the first (sorted)
                # match deterministically so the same CID always resolves
                # the same way across runs.
                for ext in known_exts:
                    matches = sorted(search_dir.glob(f"{safe_name}_*.{ext}"))
                    if matches:
            ...
            if not local_file:
                # No local file. The CID may still be reachable on the IPFS
                # network — try a network pin as a fallback rather than
                # silently skipping.
                try:
                    pin_result = subprocess.run(
                        ["ipfs", "pin", "add", "--timeout=60s", cid],
                        env=ipfs_env, capture_output=True, text=True, timeout=75
                    )
                    if pin_result.returncode == 0:
                        actual_pinned_cid = _extract_pinned_cid(pin_result.stdout, base_cid)
                        pinned += 1
                        pinned_cids.add(base_cid)
                        if actual_pinned_cid and actual_pinned_cid != base_cid:
                            pinned_cids.add(actual_pinned_cid)
                            leaf_pins_run[base_cid] = {
                                "leaf_cid": actual_pinned_cid,
                                "filename": None,
            ...
            # Pin the ORIGINAL CID directly. This is the CID referenced by the
            # NFT token — pinning it makes the token's image resolvable on IPFS.
            # We compute the hash of our local file first to verify it matches
            # the original; if it doesn't, the local file can't satisfy the
            # original CID and we report it as an error rather than silently
            # pinning a different CID.
            try:
                hash_result = subprocess.run(
                    ["ipfs", "add", "-Q", "--only-hash", str(local_file)],
                    env=ipfs_env, capture_output=True, text=True, timeout=60
                )
                computed_cid = hash_result.stdout.strip() if hash_result.returncode == 0 else ""
            except Exception:
                computed_cid = ""

            # Full CSV CID (may include a /path suffix); what we want pinned
            target_cid = cid

            if computed_cid and computed_cid == base_cid and '/' not in target_cid:
                # Fast path: content hashes match the original CID. Add the
                # file (pinned by default) — IPFS will use the same CID.
                try:
                    result = subprocess.run(
                        ["ipfs", "add", "-Q", "--pin=true", str(local_file)],
                        env=ipfs_env, capture_output=True, text=True, timeout=120
                    )
                    if result.returncode == 0 and result.stdout.strip() == base_cid:
                        pinned += 1
                        pinned_cids.add(base_cid)
                    else:
                        error_count += 1
                        error_list.append(f"{cid}: add returned unexpected CID")
                except Exception as e:
                    error_count += 1
                    error_list.append(f"{cid}: {str(e)}")
            else:
                # Content-hash mismatch (or directory-path CID that can't be
                # verified locally — typical when our local file is a leaf
                # extracted from a wrapper directory CID). Ask IPFS to pin
                # the original CID, which fetches it from the network. Track
                # for a retry pass — single-attempt timeouts on directory
                # wrappers are common and most resolve on a second try.
                try:
                    pin_result = subprocess.run(
                        ["ipfs", "pin", "add", "--timeout=60s", target_cid],
                        env=ipfs_env, capture_output=True, text=True, timeout=75
                    )
                    if pin_result.returncode == 0:
                        # When pinning a path-style CID like Qm.../file.png,
                        # IPFS resolves the path and pins the LEAF's CID, not
                        # the wrapper. The actual pinned CID appears in stdout
                        # ("pinned <CID> recursively"). Capture it so the
                        # status endpoint can credit this as a leaf-pin —
                        # otherwise it'd query `ipfs pin ls` for the wrapper
                        # and find nothing.
                        actual_pinned_cid = _extract_pinned_cid(pin_result.stdout, base_cid)
                        pinned += 1
                        pinned_cids.add(base_cid)
                        if actual_pinned_cid and actual_pinned_cid != base_cid:
                            ...
                            leaf_pins_run[base_cid] = {
                                "leaf_cid": actual_pinned_cid,
                                "filename": Path(local_file).name if local_file else None,
                                "pinned_at": datetime.utcnow().isoformat() + "Z",
                                "wrapper_err": "",
                                "source": "path-resolution",
                            }
                    else:
                        err_msg = pin_result.stderr.strip() or "pin failed"
                        retry_queue.append((cid, base_cid, err_msg, local_file))
                except Exception as e:
                    retry_queue.append((cid, base_cid, str(e), local_file))

        # Retry pass — re-attempt every CID that timed out / errored on the
        # first pass with a longer per-pin timeout. Many directory-wrapper
        # CIDs that look unreachable on the first try resolve on a second
        # attempt because IPFS bitswap connections warm up over time.
        for retry_round, (per_pin_timeout, subprocess_timeout) in enumerate([(120, 140), (180, 200)], start=1):
            if not retry_queue:
                break
            still_failing = []
            for idx, (cid, base_cid, prev_err, local_file) in enumerate(retry_queue):
                short = cid[:12] + "..."
                write_progress({
                    "status": "running", "total": total, "processed": total,
                    "pinned": pinned, "skipped": skipped, "not_found": not_found,
                    "errors": len(still_failing), "current_cid": short,
                    "phase": f"Retry pass {retry_round} ({idx + 1}/{len(retry_queue)})..."
                })
                try:
                    pin_result = subprocess.run(
                        ["ipfs", "pin", "add", f"--timeout={per_pin_timeout}s", cid],
                        env=ipfs_env, capture_output=True, text=True,
                    ...
                    if pin_result.returncode == 0:
                        ...
                        err_msg = pin_result.stderr.strip() or prev_err
                        still_failing.append((cid, base_cid, err_msg, local_file))
                except Exception as e:
                    still_failing.append((cid, base_cid, str(e), local_file))
            retry_queue = still_failing

        # Leaf-pin fallback. The wrapper CID is unreachable on the IPFS
        # network — likely a directory CID whose siblings aren't being
        # served by anyone right now — so we pin the local file's *own*
        # content CID. The user's content is preserved (under a different
        # CID), and we record the wrapper→leaf mapping so the status
        # endpoint can credit this as "pinned as leaf" and the UI can
        # explain the situation.
        for cid, base_cid, err_msg, local_file in retry_queue:
            if not local_file or not Path(local_file).exists():
                unreachable_wrappers.append({
                    "cid": cid, "reason": err_msg, "local_file": None
                })
                error_count += 1
                error_list.append(f"{cid}: {err_msg}")
                continue
            # Hash the leaf and pin its own content CID
            try:
                hash_result = subprocess.run(
                    ["ipfs", "add", "-Q", "--pin=true", str(local_file)],
                    env=ipfs_env, capture_output=True, text=True, timeout=120
                )
                leaf_cid = hash_result.stdout.strip() if hash_result.returncode == 0 else ""
            except Exception:
                leaf_cid = ""
            if leaf_cid:
                pinned_cids.add(leaf_cid)
                leaf_pins_run[base_cid] = {
                    "leaf_cid": leaf_cid,
                ...
                unreachable_wrappers.append({
                    "cid": cid, "reason": err_msg,
                    "local_file": Path(local_file).name,
                })
                error_count += 1
                error_list.append(f"{cid}: {err_msg}")

        # Persist leaf pins to the CSV sidecar JSON, merged with any prior
        # state (so retries don't lose old leaf-pin records).
        if leaf_pins_run:
            sidecar_path = (CSV_LIBRARY_DIR / filename).with_suffix('.json')
            try:
                sidecar = json.loads(sidecar_path.read_text()) if sidecar_path.exists() else {}
                existing = sidecar.get("leaf_pins", {}) or {}
                existing.update(leaf_pins_run)
                sidecar["leaf_pins"] = existing
        ...
        write_progress({
            ...
            "leaf_pinned": len(leaf_pins_run),
            "leaf_pins": leaf_pins_run,
            "unreachable_wrappers": unreachable_wrappers,
...
def save_to_gallery():
    ...
    try:
        ...
        if gen_type == "gazer":
            ...
            generator_url = f"http://127.0.0.1/lab.html?type={gen_type}&id={gen_id}&fullscreen=1{mode_param}"
...
def scan_nft_metadata():
    ...
    try:
        ...
        collections = set()
        artists = set()
        # Track which CSVs are currently flagged "don't auto-add to gallery".
        # Newly indexed files from these CSVs get added to hidden-nfts.json
        # so the gallery slideshow doesn't pick them up automatically.
        suppressed_csvs = set()
        try:
            for sidecar in CSV_LIBRARY_DIR.glob("*.json"):
                try:
                    sm = json.loads(sidecar.read_text())
                except Exception:
                    continue
                if sm.get("auto_add_to_gallery") is False:
                    suppressed_csvs.add(sidecar.with_suffix(".csv").name)
        except Exception:
            pass
        # Snapshot of cache keys before this scan adds anything; used after
        # the scan to identify newly indexed files for the auto-hide check.
        preexisting_filenames = set(cache.get("nfts", {}).keys())
        ...
        # Auto-hide newly indexed files from CSVs flagged "don't auto-add to
        # gallery". Existing files (already in cache before this scan) are
        # left alone — flipping the toggle should NOT retroactively hide art
        # the user previously saw and curated.
        wallet_hidden_count = 0
        if suppressed_csvs:
            cached_nfts = cache.get("nfts", {})
            new_to_hide = []
            for filename, entry in cached_nfts.items():
                if filename in preexisting_filenames:
                    continue
                src_csv = entry.get("source_csv", "")
                if src_csv and src_csv in suppressed_csvs:
                    new_to_hide.append(filename)
            if new_to_hide:
                hidden_file = Path("/opt/vernis/hidden-nfts.json")
                hidden = []
                if hidden_file.exists():
                    try:
                        ...
                        pass
                hidden_set = set(hidden)
                added = [fn for fn in new_to_hide if fn not in hidden_set]
                if added:
                    hidden = sorted(hidden_set | set(added))
                    with open(hidden_file, 'w') as f:
                        json.dump(hidden, f, indent=2)
                    wallet_hidden_count = len(added)
        ...
        return jsonify({
            ...
            "wallet_auto_hidden": wallet_hidden_count,
...
_OPENSEA_DEFAULT_KEY = ""  # scrubbed for public source — set via Settings UI or OPENSEA_API_KEY env var
...
@app.route("/api/collection/<path:filename>/auto-add-gallery", methods=["POST"])
def collection_auto_add_gallery(filename):
    """Enable or disable auto-add-to-gallery for a wallet-sourced collection.
    When disabled, newly downloaded files from this CSV are added to
    hidden-nfts.json by the metadata scan, so they don't appear in the
    gallery slideshow until the user manually unhides them in Manage Art."""
    if '/' in filename or '..' in filename or not filename.endswith('.csv'):
        return jsonify({"error": "Invalid filename"}), 400
    ...
    meta['auto_add_to_gallery'] = enabled
    ...
    return jsonify({"ok": True, "auto_add_to_gallery": enabled})
...
def remote_command():
    ...
    try:
        ...
        if command == 'easter_egg':
            ...
            url = f"http://127.0.0.1/lab.html?type={egg_type}&id={egg_id}&fullscreen=1&return=gallery&cycle={egg_cycle}"
    ...
    try:
        ...
        def _show_and_return(url, d):
            ...
            try:
                ...
                time.sleep(d)
                navigate_browser_cdp('http://127.0.0.1/gallery.html')
            except Exception as e:
...
def remote_start_gallery():
    ...
    try:
        success, error = navigate_browser('http://127.0.0.1/gallery.html')
        if success:
...
def remote_stop_gallery():
    ...
    try:
        success, error = navigate_browser('http://127.0.0.1/')
        if success:
...
def _write_privileged(path, content):
    """Write to a file that may require root. Try direct, then unlink+recreate, then sudo tee.
    Why: service runs as non-root on some installs. Legacy root-owned JSON configs in
    /opt/vernis can be replaced by unlinking (parent dir is user-writable). Sysfs trip points
    and /boot/firmware/config.txt need `sudo tee`, already authorized in /etc/sudoers.d/vernis-api."""
    ...
    if result.returncode != 0:
        raise PermissionError(
            f"sudo tee {path_str} failed: {result.stderr.decode(errors='ignore').strip() or 'non-zero exit'}"
        )

def save_fan_config(config):
    """Save fan configuration"""
    FAN_CONFIG_FILE.parent.mkdir(parents=True, exist_ok=True)
    _write_privileged(FAN_CONFIG_FILE, json.dumps(config, indent=2) + "\n")
...
def apply_fan_live(mode):
    ...
    try:
        ...
        if pwm_path:
            ...
            if mode == "full":
                _write_privileged(cur_state_path, str(max_state) + "\n")
            elif mode == "off":
                _write_privileged(cur_state_path, "0\n")
            else:
                ...
                _write_privileged(cur_state_path, f"{level}\n")
            ...
            if ws_url:
                ...
                if "connect.html" in current_url:
                    ...
                    ws.send(json.dumps({
                        "id": 1,
                        "method": "Page.navigate",
                        "params": {"url": f"http://127.0.0.1/connect.html?tab=bluetooth&pairing=1&return={return_path}"}
                    }))
                ws.recv()
                ws.close()
    except Exception as e:
