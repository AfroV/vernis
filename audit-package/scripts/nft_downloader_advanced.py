...
class AdvancedNFTDownloader:
    ...
    def process_csv(self, csv_path):
        ...
        # Tag ONLY files that THIS run's CSV references (run_identifiers), not
        # the global self.downloaded set — that set gets seeded with every
        # existing file in the output dir at startup, which would let one
        # collection's downloader steal source-map entries from every other
        # collection sharing the same NFT directory.
        if self.source_csv and (self.run_identifiers or self.saved_files):
            map_file = self.output_dir / "nft-source-map.json"
            try:
                source_map = json.loads(map_file.read_text()) if map_file.exists() else {}
                mapped_count = 0
                extensions = [".gif", ".png", ".jpg", ".jpeg", ".mp4", ".glb", ".html", ".bin", ".webp", ".svg", ".avif", ".json", ".txt"]
                for identifier in self.run_identifiers:
                    if identifier in self.failed:
                        continue
                    # Find actual filename on disk — try bare and suffixed variants.
                    found = False
                    for ext in extensions:
                        ...
                        if fpath.exists():
                            ...
                            break
                    if not found:
                        # Match <identifier>_*.<ext> too (e.g. <CID>_<tokenid>.json)
                        for ext in extensions:
