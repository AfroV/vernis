...
# Single-instance guard. Without --user-data-dir, a second `chromium <url>`
# invocation talks to the first via IPC and opens <url> as a NEW TAB. That
# created the boot-into-home bug on devices where labwc+watchdog both ran
# the launcher in quick succession.
LOCK=/tmp/vernis-kiosk-launcher.lock
exec 9>"$LOCK"
if ! flock -n 9; then
    echo "[$(date)] Another kiosk-launcher is running; bailing."
    exit 0
fi
# If a healthy kiosk Chromium is already up, do nothing — the watchdog will
# relaunch us if the renderer dies.
if pgrep -af 'chromium.*--kiosk' | grep -q 'http'; then
    echo "[$(date)] Chromium kiosk already running; skipping launch."
    exit 0
fi

# We only reach here when no kiosk Chromium is up. Kill stray panels/icons/wizards
# and any zombie chromium fragments before relaunching.
...
while [ $WAITED -lt $MAX_WAIT ]; do
    # Probe whichever scheme Caddy is currently serving: HTTPS if enabled
    # (via Settings → HTTPS), else plain HTTP. Without this dual check the
    # launcher waits the full timeout when HTTPS is off, slowing kiosk boot.
    if curl -sk -o /dev/null -w "%{http_code}" https://localhost/favicon.svg 2>/dev/null | grep -q "200" \
       || curl -s  -o /dev/null -w "%{http_code}" http://localhost/favicon.svg  2>/dev/null | grep -q "200"; then
